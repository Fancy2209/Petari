        ...
        T atan2_(T y, T x) const {
            if (y >= 0.0f) {
                if (x >= 0.0f) {
                    ...
                    return TAngleConstant_< T >::RADIAN_DEG090() - get_(x, y);
                }
                x = -x;
                if (x < y) {
                    return TAngleConstant_< T >::RADIAN_DEG090() + get_(x, y);
                }
                return TAngleConstant_< T >::RADIAN_DEG180() - get_(y, x);
            }
            f32 negY = -y;
            if (x < 0.0f) {
                x = -x;
                if (x >= negY) {
                    return -TAngleConstant_< T >::RADIAN_DEG180() + get_(negY, x);
                }
                return -TAngleConstant_< T >::RADIAN_DEG090() - get_(x, negY);
            }
            if (x < negY) {
                return -TAngleConstant_< T >::RADIAN_DEG090() + get_(x, negY);
            }
